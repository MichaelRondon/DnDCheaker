...
public class AbilityManager extends ACheckManager {
    ...
    public void addToAbility(Ability.AbilityName name, int valueToAdd)
            throws GeneralException {
        if (this.iBasicData.wasCreated()) {
            Object object = this.iBasicData
                    .getDescProperty(DescProperty.AVAILABLE_ABILITY_POINTS
                            .toString());
            if (!(object instanceof Integer) || (Integer) object < valueToAdd) {
                throw new GeneralException(
                        "You don't have enought available ability points!");
            }
            int availablePoints = (Integer) object - valueToAdd;
            this.iBasicData.putDescProperty(
                    DescProperty.AVAILABLE_ABILITY_POINTS.toString(),
                    availablePoints);
        }
    ...
    public void adjustMinIntelligence() {
        Ability intelligence = (Ability) super
                .getProperty(Ability.AbilityName.INTELLIGENCE);
    ...
    public void setAbility(Ability.AbilityName name, int value) {
        if (!this.iBasicData.wasCreated()) {
            super.setProperty(new Ability(name, value, this.iBasicData));
        }
    }
}
