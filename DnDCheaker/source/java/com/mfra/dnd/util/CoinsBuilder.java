...
public class CoinsBuilder implements Serializable {
	...
	private void readExpression(String expression) {
		...
		if (expression.contains("p")) {
			...
			this.platinum = new BigDecimal(stringTokenizer.nextToken().trim()).intValue();
		...
		if (expression.contains("g")) {
			...
			this.gold = new BigDecimal(stringTokenizer.nextToken().trim()).intValue();
			if (stringTokenizer.hasMoreTokens()) {
				expression = stringTokenizer.nextToken();
			}
		}
		if (expression.contains("s")) {
			...
			this.silver = new BigDecimal(stringTokenizer.nextToken().trim()).intValue();
			if (stringTokenizer.hasMoreTokens()) {
				expression = stringTokenizer.nextToken();
			}
		}
		if (expression.contains("c")) {
			...
			this.copper = new BigDecimal(stringTokenizer.nextToken().trim()).intValue();
		}
	}
