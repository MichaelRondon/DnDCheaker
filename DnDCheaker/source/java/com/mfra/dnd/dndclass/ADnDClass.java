...
public abstract class ADnDClass extends
    ...
    public enum Level {
		...
		*/
        ...
        public BigDecimal getMaxNOClassSkillRanks() {
            return new BigDecimal(this.getMaxClassSkillRanks())
        ...
        public int getNextRequiredXP() {
            return this.getRequiredXP(this.level + 1);
        }
        ...
        private int getRequiredXP(int level) {
            ...
            if (level == 0) {
                ...
                resp = (level - 1) * 1000 + this.getRequiredXP(level - 1);
            }
            return resp;
        }
    ...
    public void addXPPoints(int xpToAdd) {
        this.xPPoints += xpToAdd;
        if (this.xPPoints >= this.getLevel().getNextRequiredXP()) {
            // TODO
    ...
    public int[] getBaseAttackBonus() {
        int[] resp;
        if (this.getName().getBaseAttackBonusType()
                .equals(AttackBonusTypes.POOR)) {
            resp = this.getLevel().getBaseAttackBonusPoor();
        } else if (this.getName().getBaseAttackBonusType()
                .equals(AttackBonusTypes.AVERAGE)) {
    ...
    public CoinsBuilder getRamdomCoins() {
        ...
        if (!dnDClassName.equals(DnDClassName.MONK)) {
            ...
            IDice rollDice = d4.rollDice(null, dnDClassName.baseRndmStrtCoins,
                    0);
    ...
    public String toString() {
        ...
        return DnDUtil.getInstance().simpleConcat(super.toString(), " Level:",
                this.getLevel().level, " (", this.getXPPoints(), "/",
                this.getLevel().getNextRequiredXP(), ")");
    }
    ...
    protected void updateSkillPoints() {
        ACheckeable aCheckeable = this.iBasicData
                .getCheckProperty(Ability.AbilityName.INTELLIGENCE);
        ...
        Integer skillPoints = (Integer) this.iBasicData
                .getDescProperty(Skill.SKILL_POINTS);
