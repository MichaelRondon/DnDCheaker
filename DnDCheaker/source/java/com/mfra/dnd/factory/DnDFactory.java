...
public class DnDFactory implements Serializable {
	 ...
	 */
	public DnDFactory(HashMap<Enum<?>, ACheckeable> checkProperties, HashMap<String, Object> descProperties) {
	...
	public ACharacterElement<?> getCharacterElementInstance(Class<?> classTo, Enum<?> name) {
		...
		try {
			Constructor<?> declaredConstructor = classTo.getDeclaredConstructor(name.getClass(),
					this.checkProperties.getClass(), this.descProperties.getClass());
			Object newInstance = declaredConstructor.newInstance(name, this.checkProperties, this.descProperties);
